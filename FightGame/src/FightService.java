...
public class FightService {
    ...
    public void fight(Unit unit1, Unit unit2) {
        ...
        while (unit1.getHealth() > 0 && unit2.getHealth() > 0){
            ...
            log.add(FightMessagesUtil.buildPhysicalDamageMessage(unit1, physicalDamage1, unit2));

            if(random.nextInt(100) < 30){
                unit2.magicHit(magicDamage1);
                log.add(FightMessagesUtil.buildMagicDamageMessage(unit1, magicDamage1, unit2));
            }
            int physicalDamage2 = unit2.getPhysicalDamage();
            int magicDamage2 = unit2.getMagicDamage();
            unit1.physicalHit(physicalDamage2);
            log.add(FightMessagesUtil.buildPhysicalDamageMessage(unit2, physicalDamage2, unit1));
            if(random.nextInt(100) < 30){
                unit1.magicHit(magicDamage2);
                log.add(FightMessagesUtil.buildMagicDamageMessage(unit2, magicDamage2, unit1));
            }
        }
